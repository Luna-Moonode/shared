...
$(document).ready(function () {
	...
	let $underline = $("#wrapper .underline");
	let refreshLength;

	let regZhongWen = new RegExp("[\u4E00-\u9FA5]+");
	let regNumAndLetter = new RegExp("^[0-9a-zA-Z]+$");
	let regPie = new RegExp("[']"); //匹配一撇那个符号   '
	let nameEffective = 0;
	...
	$button.click(function () {
		if (nameEffective != 1) {
			...
			let name = $input.val();
			let postStr = '{"username":' + name + '}';
			let postJSON = $.parseJSON(postStr);
			//console.log(postJSON);
			$.post("http://127.0.0.1:8000/api/outcome", postJSON, function (data, textStatus, jqXHR) {
				console.log(data);
			});
		}
	});
